    ...
    if (this.props.display === 'feedback' && this.state.display === 'stars') {
      ...
      return (
        <div id="feedback-modal">
          <div className="feedback-exit">
            <img height="10px" src={exit} onClick={this.handleExit}></img>
          </div>
          <div className="feedback-logo">
            <img height="30px" src={login_logo}></img>
          </div>
          <br></br>
          ...
          <div className="stars-container">
            <div
              className="star"
              onClick={() => this.submitStars(1)}
              onMouseEnter={() => this.handleStars(1)}
              onMouseLeave={() => this.handleStars(0)}
            >
              {(this.state.stars || this.state.starsSubmitted) >= 1 ? (
            ...
            <div
              className="star"
              onClick={() => this.submitStars(2)}
              onMouseEnter={() => this.handleStars(2)}
              onMouseLeave={() => this.handleStars(0)}
            >
              {(this.state.stars || this.state.starsSubmitted) >= 2 ? (
                <img src={filledStar}></img>
              ) : (
                <img src={star}></img>
              )}
            ...
            <div
              className="star"
              onClick={() => this.submitStars(3)}
              onMouseEnter={() => this.handleStars(3)}
              onMouseLeave={() => this.handleStars(0)}
            >
              {(this.state.stars || this.state.starsSubmitted) >= 3 ? (
                <img src={filledStar}></img>
              ) : (
                <img src={star}></img>
              )}
            ...
            <div
              className="star"
              onClick={() => this.submitStars(4)}
              onMouseEnter={() => this.handleStars(4)}
              onMouseLeave={() => this.handleStars(0)}
            >
              {(this.state.stars || this.state.starsSubmitted) >= 4 ? (
                <img src={filledStar}></img>
              ) : (
                <img src={star}></img>
              )}
            ...
            <div
              className="star"
              onClick={() => this.submitStars(5)}
              onMouseEnter={() => this.handleStars(5)}
              onMouseLeave={() => this.handleStars(0)}
            >
              {(this.state.stars || this.state.starsSubmitted) >= 5 ? (
                <img src={filledStar}></img>
              ) : (
                <img src={star}></img>
              )}
          ...
          {this.state.feedbackTopic ? (
            <FeedbackForm exitForm={this.exitForm} />
          ) : null}
        </div>
      );
    } else if (
      this.props.display === 'feedback' &&
      this.state.display === 'exit'
    ) {
      return (
  ...
  render() {
    return (
      <div className="topic-form">
        ...
        <div className="feedback-text">
          WHAT WOULD YOU LIKE TO TELL US MORE ABOUT?
        </div>
        ...
        {this.state.display === 'default' ? (
          <DefaultSubmission exitForm={this.props.exitForm} />
        ) : null}
        {this.state.display === 'product' ? (
          <ProductSubmission exitForm={this.props.exitForm} />
        ) : null}
        {this.state.display === 'experience' ? (
          <ExperienceSubmission exitForm={this.props.exitForm} />
        ) : null}
      </div>
    );
  }
...
function ExperienceSubmission(props) {
  var dropdown = [
    ...
    'Research/Compare Products in Person',
    'Try on/out a Produict',
    'Pick-Up an Online Order',
    'Attend a Store Event',
    'Get Advice from a Store Athlete',
    'None of the Above'
  ];
  ...
  return (
    <div className="experience-submission">
      <div className="feedback-text">
        DESCRIBE YOUR EXPERIENCE. WHAT WAS POSITIVE? WHAT CAN WE IMPROVE?
      </div>
      <textarea className="feedback-textarea"></textarea>
      <div className="feedback-text">
        WHAT WAS YOUR GOAL FOR YOUR STORE VISIT?
      </div>
      <select id="topic-dropdown">
      ...
      <table>
        <tr className="radio-rating-container">
          <label className="radio-rating-num">
            {' '}
            1<br></br>
  ...
  return (
    <div className="default-submission">
      ...
      <div className="assistance-text">
        We are unable to respond to requests submitted here. If you need
        assistance with your Nike Product or Services please click here to
        contact us.
      </div>
